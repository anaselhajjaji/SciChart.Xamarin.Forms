using System;
using SciChart.Xamarin.Views.Numerics.GenericMath;
...
{
    /// <summary>
    /// Abstract base implementation of <see cref="IRange"/>, used throughout SciChart for visible, data and index range calculations
    /// </summary>
    /// <typeparam name="T">The typeparameter of the range, e.g. <see cref="System.Double"/></typeparam>
    public abstract class Range<T> : NotifyPropertyChangedImpl, IRange<T> where T : IComparable
    {
        private T _min;
        private T _max;
        private readonly bool _instantiated = false;
        ...
        /// <remarks></remarks>
        public virtual bool IsDefined { get { return ComparableUtil.IsDefined(Max) && ComparableUtil.IsDefined(Min); } }
        ...
        public T Min
        {
            ...
            set
            {
                ...
                if (!_min.Equals(value))
                {
                    ...
                    // Prevents slow call to OnPropertyChanged before instantiation is complete
                    if (_instantiated)
        ...
        public IRange<T> SetMinMaxT(T min, T max)
        {
